...
DEALER_RECEIPT_SCHEMA = SchemaDefinition(
    ...
    fields=[
        ...
        FieldSpec(key="payment_mode", field_type="string", required=False, description="Payment mode exactly as printed, for example cash, cheque, demand draft (DD), NEFT, RTGS, IMPS, bank transfer, banker's order (BO), pay order (PO), trade-in adjustment, refund, UPI or card"),
    ...
    prompt_notes=[
        ...
        "payment_mode also covers a receipt explicitly noting the amount is a trade-in/exchange adjustment or a refund, not only a money-transfer channel -- extract whichever label the receipt itself uses.",
