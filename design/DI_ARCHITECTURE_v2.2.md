...
### Access/API
Provider-neutral OIDC/JWT authentication, canonical RBAC permissions, registered-device checks for USER actors, correlation middleware, stable Problem/error codes, native Subject Registry APIs, Subject-centric Document APIs, configuration, verification and operations APIs. Each OpenAPI operation declares `x-required-permissions`; clients branch on stable Problem `code`.
...
### Processing
PostgreSQL jobs, deterministic candidate-set formation, classification, profile resolution, provider-neutral extraction, normalization, deterministic validation, confidence scoring and one EOD retry for retryable first failure. Candidate formation uses all effective processing-ready Tenant/global Document Types; Requirement Profile and caller hint provide context but do not exclude ADDITIONAL evidence.

### Query/Verification
Subject completeness, exceptions, upload quality, verification queue and human-correction lineage.

### Audit/Observability
Entity-scoped hash-chained Tenant audit events, structured logs, metrics and one end-to-end correlation ID. Audit writes for unrelated entities do not serialize on one Tenant chain head. Provider request IDs remain separate.
...
## 8. Security and error contracts

- JWT/RBAC source of truth: `docs/DI_SECURITY_RBAC_v2.2.md` and `security/DI_RBAC_v2.2.yaml`.
- Problem/error source of truth: `docs/DI_ERROR_CATALOG_v2.2.md` and `api/DI_ERROR_CATALOG_v2.2.yaml`.
- Classification source of truth: `docs/DI_CLASSIFICATION_v2.2.md`.
- Audit-chain source of truth: `docs/DI_AUDIT_MODEL_v2.2.md`.
